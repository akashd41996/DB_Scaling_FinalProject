...
queries = [
    ...
    FROM `dba-final-project-gcp.without_vm_upload.HV_Freq_Table` a
    JOIN `dba-final-project-gcp.without_vm_upload.HV_Freq_Table` b ON a.Hvfhs_license_num = b.Hvfhs_license_num
    ...
    """
        ...
        """,
    """
        SELECT origin.PULocationID, COUNT(destination.DOLocationID) as DestinationCount
        FROM `dba-final-project-gcp.without_vm_upload.HV_Freq_Table` origin
        JOIN `dba-final-project-gcp.without_vm_upload.HV_Freq_Table` destination
        ON origin.PULocationID = destination.DOLocationID
        GROUP BY origin.PULocationID
        LIMIT 1000
        """
